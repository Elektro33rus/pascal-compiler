...
public final class Parser {
    ...
    */
    private static void funcDeclaration() {
        ...
        if (currentToken.getTokenType().equals("TK_FUNCTION")) {
            ...
            genAddress(0);
            if (SymbolTable.lookup(functionName, "Global") == null) { //if functionname already insert
                SymbolTable.insert(symbol);
            }
            else {
            	throw new Error(String.format("function '"+functionName + "' already is defined!"));
            }
    ...
    */
    ...
    private static Stack<String> nameVarFunc = new Stack<String>();
    ...
    public static void varDeclarationsFunc() {
        while(true) {
            ...
            while ("TK_IDENTIFIER".equals(currentToken.getTokenType())) {
                ...
                nameVarFunc.push(currentToken.getTokenValue());
                match("TK_A_VAR");
    ...
    private static void adder() {
        ...
        if (symbol21 != null) {
                // variable
                currentToken.setTokenType("TK_A_VAR");
                t = symbol21.getDataType();
                genOpCode(OP_CODE.PUSH);
                int temp = symbol21.getAddress();
                Symbol t1 = SymbolTable.lookup(nameVarFunc.firstElement(), "FUNCVAR"+Region);
                nameVarFunc.remove(nameVarFunc.firstElement());
