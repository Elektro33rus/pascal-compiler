...
public class Simulator {
    ...
    private static void ifend() {
    	...
    	String temp = (String) giveMeNumberVar();
    	forIfthen.set(schet, new iffen(forIfthen.get(schet).ifen, forIfthen.get(schet).number));
    	if (forIfelse.size()==schet+1) {
    		AllProgram+="\n"+forIfthen.get(schet).ifen+temp+"\n"+forIfelse.get(schet).ifen+"br label %"+temp+"\n\n;Label %"+temp+" ifend\n";
    		forIfelse.add(new iffen("", ""));
    	}
    	else {
    		AllProgram+=temp+"\n"+forIfthen.get(schet).ifen+"br label %"+temp+"\n\n;Label %"+temp+" ifend\n";
    	}
    }
    ...
    private static String AllProgram="@.str = private unnamed_addr constant [4 x i8] c\"%i\\0A\\00\"\n\n"
    ...
    private static int repeatJumpFunc;
