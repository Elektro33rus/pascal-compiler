...
public final class Parser {
    enum TYPE {
        I, R, B, C, S, P, F, L, A     // integer, real, boolean, char, string, procedure, label, array
    }
        ...
        while (true) {
            switch (currentToken.getTokenType()) {
                ...
                case "TK_BEGIN":
                    return;
                case "TK_IDENTIFIER":
                	throw new Error(String.format(currentToken.getTokenValue()+" sintax error"));
            }
        }
    }
        ...
        while(true) {
            ...
            for (Token label : labelsArrayList) {
                Symbol symbol = new Symbol(label.getTokenValue(),
                        ...
                        TYPE.L,
                        0);
                if (SymbolTable.lookup(label.getTokenValue(), Region) == null) {
    ...
    */
    private static void funcDeclaration() {
        ...
        if (currentToken.getTokenType().equals("TK_FUNCTION")) {
            ...
            String functionName = currentToken.getTokenValue();
            match("TK_A_FUNC");
            ...
            genAddress(0);
            if (SymbolTable.lookup(functionName, Region) == null) {
                SymbolTable.insert(symbol);
            }
            else {
            	throw new Error(String.format(functionName + " is defined!"));
            }
        ...
        if (indexType1 != indexType2){
            ...
            switch (indexType1) {
                case I:
                    ...
                    Symbol firstIntArray = SymbolTable.lookup(variablesArrayList.get(0).getTokenValue(), Region);
                    if (firstIntArray != null) {
                        dp = firstIntArray.getAddress();
                    }
                    for (Token var: variablesArrayList) {
                        Symbol symbol = SymbolTable.lookup(var.getTokenValue(), Region);
                        if (symbol != null){
                            int elementSize = 4;
                ...
                case C:
                    ...
                    Symbol firstCharArray = SymbolTable.lookup(variablesArrayList.get(0).getTokenValue(), Region);
    ...
    */
    public static void caseStat() {
        ...
        while(currentToken.getTokenType().equals("TK_INTLIT") ||
            ...
            if (!currentToken.getTokenValue().equals("TK_END")){
                Symbol symbol = SymbolTable.lookup(eToken.getTokenValue(), Region);
                if (symbol != null) {
                    genOpCode(OP_CODE.PUSH);
    ...
    private static void handleArrayAccess(Symbol symbol) {
        ...
        match("TK_OPEN_SQUARE_BRACKET");
        TYPE t;
        Symbol varSymbol = SymbolTable.lookup(currentToken.getTokenValue(), Region);
        if (varSymbol != null) {
