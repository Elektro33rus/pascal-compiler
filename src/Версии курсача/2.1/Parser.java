...
POPRESULT,
PUSHRESULT,
REPLACERESULT, CALLINT, CALLREAL
    }
    ...
    private static void callfunc() {
        ...
        while (currentToken.getTokenType().equals("TK_IDENTIFIER") || 
        	...
        	kolvo++;
	    	Symbol parametr = SymbolTable.lookup(currentToken.getTokenValue(), "FUNCVAR"+Region);
	    	if (parametr == null) {
	    		parametr = SymbolTable.lookup(currentToken.getTokenValue(), Region);
	    	}
	        if (parametr == null) {
	        	parametr =  SymbolTable.lookup(currentToken.getTokenValue(), "Global");
	        }
	        TYPE t;
	        if (parametr != null) {
	                currentToken.setTokenType("TK_A_VAR");
                ...
                if (symbol != null) {
                    ...
                    if (symbol.getTokenType().equals("TK_A_VAR")) {
                    	...
                    	if (varFunctionResult.getDataType()==TYPE.I)
                    		genOpCode(OP_CODE.CALLINT);
                    	else
                    		if (varFunctionResult.getDataType()==TYPE.R)
                    			genOpCode(OP_CODE.CALLREAL);
